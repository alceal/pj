...
- Auto-rename the current tmux window to the selected project's directory basename when running `pj` inside a tmux session

## [0.3.5] - 2026-04-10

### Fixed

- Shell function now runs `--list` directly instead of capturing its stdout, fixing terminal width detection and table rendering

## [0.3.4] - 2026-04-10

### Fixed

- `pj --list` path shortening now works on wide terminals by capping the path column width

## [0.3.3] - 2026-04-10

### Fixed

- `pj --list` path display now collapses middle directories with `~/.../` to fit the terminal width instead of truncating column content
...
- `pj --init` now replaces the existing shell function instead of skipping when the marker is already present, ensuring updates to the shell integration are applied
- Shell function is now installed when `ai_assistant` is configured, even if `cd_on_select` is disabled
...
- Skip GitHub remote creation when git repository does not exist (e.g., when user declines git init prompt)
